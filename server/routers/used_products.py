...
async def create_product(request: Request, background_tasks: BackgroundTasks, product: CreateUsedProduct, current_user: ShowUserWithId = Depends(get_current_user)):
    ...
    background_tasks.add_task(send_notification, tokens=devices, detail={}, type='new-used-product',
                              title='Used Product', body='User {} has added a new second hand product.'.format(current_user['full_name']))
...
async def get_products(request: Request, page: int = 1, sort: int = 0, category: str = None, search: str = None):
    ...
    if search != None:
        if sort == 0:
            products = request.app.mongodb['UsedProducts'].find(
                {"name": {"$regex": f".*{search}.*", '$options': 'i'}}).skip((page-1)*products_per_page).limit(products_per_page)
            product2 = request.app.mongodb['UsedProducts'].find(
                {"name": {"$regex": f".*{search}.*", '$options': 'i'}}).skip((page)*products_per_page).limit(products_per_page)
        elif sort == 1:
            products = request.app.mongodb['UsedProducts'].find({"name": {"$regex": f".*{search}.*", '$options': 'i'}}).sort(
                'price', 1).skip((page-1)*products_per_page).limit(products_per_page)
            product2 = request.app.mongodb['UsedProducts'].find(
                {"name": {"$regex": f".*{search}.*", '$options': 'i'}}).sort('price', 1).skip((page)*products_per_page).limit(products_per_page)
        else:
            products = request.app.mongodb['UsedProducts'].find({"name": {"$regex": f".*{search}.*", '$options': 'i'}}).sort(
                'price', -1).skip((page-1)*products_per_page).limit(products_per_page)
            product2 = request.app.mongodb['UsedProducts'].find({"name": {"$regex": f".*{search}.*", '$options': 'i'}}).sort(
                'price', -1).skip((page)*products_per_page).limit(products_per_page)
        count = 0
    ...
    if category != None:
        if sort == 0:
            products = request.app.mongodb['UsedProducts'].find({"category": category}).skip(
                (page-1)*products_per_page).limit(products_per_page)
            product2 = request.app.mongodb['UsedProducts'].find(
                {"category": category}).skip((page)*products_per_page).limit(products_per_page)
        elif sort == 1:
            products = request.app.mongodb['UsedProducts'].find({"category": category}).sort(
                'price', 1).skip((page-1)*products_per_page).limit(products_per_page)
            product2 = request.app.mongodb['UsedProducts'].find({"category": category}).sort(
                'price', 1).skip((page)*products_per_page).limit(products_per_page)
        else:
            products = request.app.mongodb['UsedProducts'].find({"category": category}).sort(
                'price', -1).skip((page-1)*products_per_page).limit(products_per_page)
    ...
    product2 = {}
    if sort == 0:
        products = request.app.mongodb['UsedProducts'].find().skip(
            (page-1)*products_per_page).limit(products_per_page)
        product2 = request.app.mongodb['UsedProducts'].find().skip(
            (page)*products_per_page).limit(products_per_page)
    elif sort == 1:
        products = request.app.mongodb['UsedProducts'].find().sort(
            'price', 1).skip((page-1)*products_per_page).limit(products_per_page)
        product2 = request.app.mongodb['UsedProducts'].find().sort(
            'price', 1).skip((page)*products_per_page).limit(products_per_page)
    else:
        products = request.app.mongodb['UsedProducts'].find().sort(
            'price', -1).skip((page-1)*products_per_page).limit(products_per_page)
...
async def get_products(request: Request, page: int = 1, sort: int = 0, category: str = None, search: str = None, current_user: ShowUserWithId = Depends(validate_admin)):
    ...
    if sort == 0:
        products = request.app.mongodb['UsedProducts'].find().sort(
            'date_added', -1).skip((page-1)*products_per_page).limit(products_per_page)
        product2 = request.app.mongodb['UsedProducts'].find().sort(
            'date_added', -1).skip((page)*products_per_page).limit(products_per_page)
    elif sort == 1:
        products = request.app.mongodb['UsedProducts'].find().sort(
            'price', 1).skip((page-1)*products_per_page).limit(products_per_page)
        product2 = request.app.mongodb['UsedProducts'].find().sort(
...
async def question_product(request: Request, questions: ProductQuestion, current_user: ShowUserWithId = Depends(get_current_user)):
    ...
    if product_check is None:
        raise HTTPException(
            status_code=404, detail=f"Product with id {pid} not found")
    print(product_check)
...
async def request_to_buy(request: Request, id: str, request_to_buy: RequestToBuy, background_tasks: BackgroundTasks, current_user: ShowUserWithId = Depends(get_current_user)):
    ...
    background_tasks.add_task(send_notification, tokens=devices, detail={'requests': product_check['requests_to_buy'], 'product': product_check},
                              type='used-product-request', title='Used Product Request', body='User {} has requested to buy {}'.format(current_user['full_name'], product_check['name']))
    return {'success': True}
