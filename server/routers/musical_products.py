...
async def get_products(request: Request, page: int = 1, sort: int = 0, category: str = None, search: str = None, admin: bool = False):
    ...
    if search != None:
        if sort == 0:
            products = request.app.mongodb['Products'].find(
                {"name": {"$regex": f".*{search}.*", '$options': 'i'}}).skip((page-1)*products_per_page).limit(products_per_page)
            product2 = request.app.mongodb['Products'].find(
                {"name": {"$regex": f".*{search}.*", '$options': 'i'}}).skip((page)*products_per_page).limit(products_per_page)
        elif sort == 1:
            products = request.app.mongodb['Products'].find({"name": {"$regex": f".*{search}.*", '$options': 'i'}}).sort(
                'price', 1).skip((page-1)*products_per_page).limit(products_per_page)
            product2 = request.app.mongodb['Products'].find({"name": {"$regex": f".*{search}.*", '$options': 'i'}}).sort(
                'price', 1).skip((page)*products_per_page).limit(products_per_page)
        elif sort == 2:
            products = request.app.mongodb['Products'].find({"name": {"$regex": f".*{search}.*", '$options': 'i'}}).sort(
                'points', -1).skip((page-1)*products_per_page).limit(products_per_page)
            product2 = request.app.mongodb['Products'].find({"name": {"$regex": f".*{search}.*", '$options': 'i'}}).sort(
                'points', -1).skip((page)*products_per_page).limit(products_per_page)
        else:
            products = request.app.mongodb['Products'].find({"name": {"$regex": f".*{search}.*", '$options': 'i'}}).sort(
                'price', -1).skip((page-1)*products_per_page).limit(products_per_page)
            product2 = request.app.mongodb['Products'].find({"name": {"$regex": f".*{search}.*", '$options': 'i'}}).sort(
    ...
    if category != None:
        if sort == 0:
            products = request.app.mongodb['Products'].find({"category": category}).skip(
                (page-1)*products_per_page).limit(products_per_page)
            product2 = request.app.mongodb['Products'].find({"category": category}).skip(
                (page)*products_per_page).limit(products_per_page)
        elif sort == 1:
            products = request.app.mongodb['Products'].find({"category": category}).sort(
                'price', 1).skip((page-1)*products_per_page).limit(products_per_page)
            product2 = request.app.mongodb['Products'].find({"category": category}).sort(
                'price', 1).skip((page)*products_per_page).limit(products_per_page)
        elif sort == 2:
            products = request.app.mongodb['Products'].find({"category": category}).sort(
                'points', -1).skip((page-1)*products_per_page).limit(products_per_page)
    ...
    if sort == 0:
        if admin:
            products = request.app.mongodb['Products'].find().skip(
                (page-1)*products_per_page).limit(products_per_page)
            product2 = request.app.mongodb['Products'].find().skip(
                (page)*products_per_page).limit(products_per_page)
        else:
            products = request.app.mongodb['Products'].find().sort(
                [('avg_rating', -1), ('_id', 1)]).skip((page-1)*products_per_page).limit(products_per_page)
            product2 = request.app.mongodb['Products'].find().sort(
                [('avg_rating', -1), ('_id', 1)]).skip((page)*products_per_page).limit(products_per_page)
    elif sort == 1:
        products = request.app.mongodb['Products'].find().sort('price', 1).skip(
            (page-1)*products_per_page).limit(products_per_page)
        product2 = request.app.mongodb['Products'].find().sort(
            'price', 1).skip((page)*products_per_page).limit(products_per_page)
    elif sort == 2:
...
async def get_products(request: Request, value: str, page: int = 0):
    ...
    if (len(products) == 0):
        raise HTTPException(
            status_code=404, detail=f"Products with query {value} not found")
