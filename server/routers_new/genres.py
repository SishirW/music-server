...
@router.get('/count')
async def get_total_genres_count(request: Request, current_user: ShowUser = Depends(get_current_user)):
    db = get_database(request)
...
@router.get('/{id}')
async def get_genre_by_id(id: str, request: Request, current_user: ShowUser = Depends(get_current_user)):
    db = get_database(request)
