...
@router.get('/')
async def get_genres(request: Request, page=1, limit=5, current_user: ShowUser = Depends(validate_admin)):
...
@router.get('/count')
async def get_total_genres_count(request: Request, current_user: ShowUser = Depends(validate_admin)):
    db = get_database(request)
...
@router.get('/{id}')
async def get_genre_by_id(id: str, request: Request, current_user: ShowUser = Depends(validate_admin)):
    db = get_database(request)
