...
@router.get('/')
async def get_venuecategory(request: Request, page=1, limit=5, current_user: ShowUser = Depends(validate_user)):
...
@router.get('/count')
async def get_total_venuecategory_count(request: Request, current_user: ShowUser = Depends(validate_user)):
    db = get_database(request)
...
@router.get('/{id}')
async def get_venuecategory_by_id(id: str, request: Request, current_user: ShowUser = Depends(validate_user)):
    db = get_database(request)
