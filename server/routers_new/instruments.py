...
from server.routers.user import validate_admin
...
@router.get('/')
async def get_instruments(request: Request, page=1, limit=5, current_user: ShowUser = Depends(validate_admin)):
...
@router.get('/count')
async def get_total_instruments_count(request: Request, current_user: ShowUser = Depends(validate_admin)):
    db = get_database(request)
...
@router.get('/{id}')
async def get_instrument_by_id(id: str, request: Request, current_user: ShowUser = Depends(validate_admin)):
    db = get_database(request)
